...
import { deleteTeam, getTeam, updateTeam, type UpdateTeamInput } from "@backend/services/teams";
...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const data = await getTeam(auth.supabase, { id });
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await ctx.params;
  const body = await readJson<UpdateTeamInput>(req);
  if (!body) {
    return fail(
      400,
  ...
  try {
    const data = await updateTeam(auth.supabase, { id, patch: body });
    return ok(data, { headers: rateLimitHeaders(auth.rateLimit ?? null) });
  } catch (e: unknown) {
    const mapped = mapToHttpError(e);
    return fail(mapped.status, mapped.error, { headers: rateLimitHeaders(auth.rateLimit ?? null) });
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    await deleteTeam(auth.supabase, { id });
